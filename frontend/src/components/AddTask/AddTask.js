...
export default function AddTask({
  ...
  const initialTask = editingTask
    ? {
        ...editingTask,
        startTime: convertBackendToFrontend(editingTask.startTime) || defaultTime || "",
        deadline: convertBackendToFrontend(editingTask.deadline) || "",
      }
    : {
        name: "",
  ...
  const handleConfirmClick = () => {
    ...
    const convertTimeFormat = (isoTime) => {
      if (!isoTime) return "";
      try {
        return isoTime.replace(/[T\s-:]/g, '').substring(0, 12);
      } catch (e) {
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          {editingTask && (
            <button
              ...
              onClick={() => onDelete(task.id,task.mode)}
            >
              Delete
            </button>
          )}
